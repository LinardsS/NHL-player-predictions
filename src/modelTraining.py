...
def trainSkaterSeasonAndTwoWkTotalsModel():
    ...
    filename = 'Linear_Regression_SeasonTwoWk' + str(r_sq_score) + '_'+ datum + '.sav'
...
def trainSkaterPlainGoalsOnlyModel():
    linreg = LinearRegression()
    conn = utils.establishDatabaseConnection("main.db")

    query = """SELECT twowk.goals as twowk_goals, sgd.goals as res_goals
            FROM   games g, skater_game_data sgd, skater_two_wk_totals twowk, skater_season_totals sst, goalie_season_totals gst, teams t
            WHERE  g.id = sgd.game_id
            and    g.date >= (?)
            and    g.date <= (?)
    ...
    #Save model
    datum = utils.getTodaysDate(format = "%Y-%m-%d",backdate = None)
#     filename = 'Linreg_SeasonPlain_SM_GOALSONLY_' + str(r_sq_score) + '_'+ datum + '.sav'
    filename = 'Linreg_SeasonPlain_SM_GOALSONLY_' + '_'+ datum + '.sav'
...
def dtTrainSkaterSeasonAndTwoWkTotalsModel():
    ...
    #Save model
    datum = utils.getTodaysDate(format = "%Y-%m-%d",backdate = None)
    filename = 'DT_' + str(md) + 'MD_FMSE_0001ccp_SeasonTwoWk_' + str(r_sq_score) + '_'+ datum + '.sav'
    print(filename)
    pickle.dump(dt, open(filename, 'wb'))

def plotDecisionTreeRegressor(model_name):
    dt = loadModel(model_name)
    feature_names = ['twowk_games_played', 'twowk_time_on_ice', 'twowk_goals', 'twowk_assists', 'twowk_first_assists',
                     'twowk_second_assists', 'twowk_points', 'twowk_ipp', 'twowk_shots', 'twowk_shooting_pct', 'twowk_ixg',
                     'twowk_icf', 'twowk_iff', 'twowk_iscf', 'twowk_ihdcf', 'twowk_rush_attempts', 'twowk_rebounds_created',
                     'twowk_penalty_minutes', 'twowk_penalties_drawn', 'twowk_giveaways', 'twowk_takeaways', 'twowk_hits',
                     ' twowk_hits_taken', 'twowk_shots_blocked', 'twowk_faceoffs_won', 'twowk_faceoffs_lost', 'twowk_faceoff_pct',
                     'sst_games_played', 'sst_time_on_ice', 'sst_goals', 'sst_assists', 'sst_first_assists', 'sst_second_assists', 
                     'sst_points', 'sst_ipp', 'sst_shots', 'sst_shooting_pct', 'sst_ixg', 'sst_icf', 'sst_iff', 'sst_iscf', 'sst_ihdcf',
                     'sst_rush_attempts', 'sst_rebounds_created', 'sst_penalty_minutes', 'sst_penalties_drawn', 'sst_giveaways', 'sst_takeaways',
                     'sst_hits', 'sst_hits_taken', 'sst_shots_blocked', 'sst_faceoffs_won', 'sst_faceoffs_lost', 'sst_faceoff_pct']
    ...
    feat_importances.nlargest(6).plot(kind='barh', fontsize=12, figsize=(5,5)).invert_yaxis()
...
#     plt.subplots_adjust(bottom=0.1, right=0.8, top=0.9)
    # create the plot
#     fig, ax = plt.subplots(figsize=(8, 6))
#     ax.plot(dt.feature_importances_)
...
def plotSimplifiedDecisionTreeRegressor(model_name):
    dt = loadModel(model_name)
    feature_names = ['twowk_goals', 'twowk_assists','twowk_shooting_pct', 'twowk_ixg',
                     'twowk_icf', 'twowk_iff', 'twowk_iscf', 'twowk_ihdcf', 'twowk_rush_attempts', 'twowk_rebounds_created',
                     'twowk_penalty_minutes', 'twowk_hits',
                     'twowk_shots_blocked', 'twowk_faceoff_pct',
                     'sst_goals', 'sst_assists', 'sst_shooting_pct', 'sst_ixg', 'sst_icf', 'sst_iff', 'sst_iscf', 'sst_ihdcf',
                     'sst_rush_attempts', 'sst_rebounds_created', 'sst_penalty_minutes', 
                     'sst_hits', 'sst_shots_blocked', 'sst_faceoff_pct']
    feature_importance = pd.DataFrame(dt.feature_importances_, index = feature_names)
...
def plotSuperSimplifiedDecisionTreeRegressor(model_name):
    dt = loadModel(model_name)
    feature_names = ['twowk_goals', 'twowk_assists','twowk_shooting_pct', 'twowk_ixg',
                     'twowk_icf', 'twowk_iff', 'twowk_iscf', 'twowk_ihdcf',
                     'sst_goals', 'sst_assists', 'sst_shooting_pct', 'sst_ixg', 'sst_icf', 'sst_iff', 'sst_iscf', 'sst_ihdcf']
    feature_importance = pd.DataFrame(dt.feature_importances_, index = feature_names)
...
def plotSuperSUPERSimplifiedDecisionTreeRegressor(model_name):
    dt = loadModel(model_name)
    feature_names = ['twowk_goals', 'sst_goals']
    feature_importance = pd.DataFrame(dt.feature_importances_, index = feature_names)
...
def trainGoalieSeasonAndTwoWkTotalsModel2Outputs():
    linreg = LinearRegression()
    conn = utils.establishDatabaseConnection("main.db")
    ...
    query = """SELECT  twowk.gp as twowk_games_played, twowk.toi as twowk_time_on_ice, (twowk.shots_against / twowk.toi * 60) as twowk_sa_p60,
                        ...
                        (gst.md_saves / gst.toi * 60) as gst_mds_p60, gst.mdsv_pct as gst_mdsv_pct, gst.mdgaa as gst_mdgaa, gst.mdgsaa as gst_mdgsaa,
                        (gst.ld_shots_against / gst.toi * 60) as gst_ldsa_p60,
                        (gst.ld_saves / gst.toi * 60) as gst_lds_p60, gst.ldsv_pct as gst_ldsv_pct, gst.ldgaa as gst_ldgaa, gst.ldgsaa as gst_ldgsaa, ggd.saves as res_saves,
                        (ggd.shots_faced - ggd.saves) as res_goals_allowed
             FROM   games g, goalie_game_data ggd, goalie_two_wk_totals twowk, goalie_season_totals gst
             WHERE  g.id = ggd.game_id
             and    g.date >= (?)
    ...
    df.replace('inf', np.nan, inplace=True)
    df.fillna(0,inplace=True)
    X = df.drop(columns = ['res_saves', 'res_goals_allowed'])
    Y = df[['res_saves', 'res_goals_allowed']]
    X_train, X_test, Y_train, Y_test = train_test_split(X, Y, test_size = 0.2, random_state=7) # 80%/20% training/test split
    linreg.fit(X_train,Y_train)
    predict = linreg.predict(X_test)
    ...
    #Save model
    datum = utils.getTodaysDate(format = "%Y-%m-%d",backdate = None)
    filename = 'Linear_Regression_GoalieSeasonTwoWkTotalsSavesAndGA' + str(r_sq_score) + '_'+ datum + '.sav'
    pickle.dump(linreg, open(filename, 'wb'))
...
def analyzeLinearRegModel(model_name):
    model = loadModel(model_name)
    print(model.intercept_, model.coef_)

def trainSkaterSeasonAndTwoWkTotalsModelSimplified():
    linreg = LinearRegression()
    conn = utils.establishDatabaseConnection("main.db")

    query = """SELECT  twowk.games_played as twowk_games_played, twowk.time_on_ice as twowk_time_on_ice, twowk.goals as twowk_goals,
                            twowk.assists as twowk_assists, twowk.shooting_pct as twowk_shooting_pct,
                            twowk.ixg as twowk_ixg, twowk.icf as twowk_icf, twowk.iff as twowk_iff,
                            twowk.iscf as twowk_iscf, twowk.ihdcf as twowk_ihdcf, twowk.rush_attempts as twowk_rush_attempts, twowk.rebounds_created as twowk_rebounds_created, 
                            twowk.penalty_minutes as twowk_penalty_minutes, twowk.penalties_drawn as twowk_penalties_drawn, 
                            twowk.giveaways as twowk_giveaways, twowk.takeaways as twowk_takeaways, twowk.hits as twowk_hits,  
                            ...
                            twowk.faceoffs_lost as twowk_faceoffs_lost, twowk.faceoff_pct as twowk_faceoff_pct,
                            sst.games_played as sst_games_played, sst.time_on_ice as sst_time_on_ice, sst.goals as sst_goals,
                            sst.assists as sst_assists, sst.shooting_pct as sst_shooting_pct,
                            sst.ixg as sst_ixg, sst.icf as sst_icf, sst.iff as sst_iff,
                            sst.iscf as sst_iscf, sst.ihdcf as sst_ihdcf, sst.rush_attempts as sst_rush_attempts, sst.rebounds_created as sst_rebounds_created, 
                            sst.penalty_minutes as sst_penalty_minutes, sst.penalties_drawn as sst_penalties_drawn, 
                            sst.giveaways as sst_giveaways, sst.takeaways as sst_takeaways, sst.hits as sst_hits,  
    ...
    #Save model
    datum = utils.getTodaysDate(format = "%Y-%m-%d",backdate = None)
    filename = 'Linear_Regression_SeasonTwoWkSIMP' + str(r_sq_score) + '_'+ datum + '.sav'
    pickle.dump(linreg, open(filename, 'wb'))

def dtTrainSkaterSeasonAndTwoWkTotalsModelSimplified():
    ccp_a = 0.01
    md = 8
    criterion = 'friedman_mse'
    dt = DecisionTreeRegressor(criterion=criterion, max_depth=md, ccp_alpha=ccp_a)
    ...
    query = """SELECT  twowk.goals as twowk_goals,
                            twowk.assists as twowk_assists,twowk.shooting_pct as twowk_shooting_pct,
                            twowk.ixg as twowk_ixg, twowk.icf as twowk_icf, twowk.iff as twowk_iff,
                            twowk.iscf as twowk_iscf, twowk.ihdcf as twowk_ihdcf, twowk.rush_attempts as twowk_rush_attempts, twowk.rebounds_created as twowk_rebounds_created, 
                            twowk.penalty_minutes as twowk_penalty_minutes, twowk.hits as twowk_hits,  
                            twowk.shots_blocked as twowk_shots_blocked,twowk.faceoff_pct as twowk_faceoff_pct,
                            sst.goals as sst_goals,
                            sst.assists as sst_assists,sst.shooting_pct as sst_shooting_pct,
                            sst.ixg as sst_ixg, sst.icf as sst_icf, sst.iff as sst_iff,
                            sst.iscf as sst_iscf, sst.ihdcf as sst_ihdcf, sst.rush_attempts as sst_rush_attempts, sst.rebounds_created as sst_rebounds_created, 
                            sst.penalty_minutes as sst_penalty_minutes,sst.hits as sst_hits,  
                            sst.shots_blocked as sst_shots_blocked, sst.faceoff_pct as sst_faceoff_pct,
                            sgd.goals as res_goals, sgd.assists as res_assists,
                            sgd.shots as res_shots, sgd.hits as res_hits, sgd.penalty_minutes as res_penalty_minutes,
                            sgd.face_off_pct as res_face_off_pct,
                            sgd.blocked_shots as res_blocked_shots, sgd.plus_minus as res_plus_minus
             FROM   games g, skater_game_data sgd, skater_two_wk_totals twowk, skater_season_totals sst
             WHERE  g.id = sgd.game_id
             and    g.date >= (?)
    ...
    X = df.drop(columns = ['res_goals', 'res_assists', 'res_shots', 'res_hits', 'res_penalty_minutes', 'res_face_off_pct', 'res_blocked_shots', 'res_plus_minus'])
    Y = df[['res_goals', 'res_assists', 'res_shots', 'res_hits', 'res_penalty_minutes', 'res_face_off_pct', 'res_blocked_shots', 'res_plus_minus']]
    X_train, X_test, Y_train, Y_test = train_test_split(X, Y, test_size = 0.2, random_state=7) # 80%/20% training/test split
    #print(train[['twowk_games_played'] + ['twowk_time_on_ice'] + ['twowk_goals'] + ['twowk_assists'] + ['twowk_first_assists'] + ['twowk_second_assists'] + ['twowk_points'] + ['twowk_ipp'] + ['twowk_shots'] + ['twowk_shooting_pct'] + ['twowk_ixg'] + ['twowk_icf'] + ['twowk_iff'] + ['twowk_iscf'] + ['twowk_ihdcf'] + ['twowk_rush_attempts'] + ['twowk_rebounds_created'] + ['twowk_penalty_minutes'] + ['twowk_penalties_drawn'] + ['twowk_giveaways'] + ['twowk_takeaways'] + ['twowk_hits'] + ['twowk_hits_taken'] + ['twowk_shots_blocked'] + ['twowk_faceoffs_won'] + ['twowk_faceoffs_lost'] + ['twowk_faceoff_pct'] + ['sst_games_played'] + ['sst_time_on_ice'] + ['sst_goals'] + ['sst_assists'] + ['sst_first_assists'] + ['sst_second_assists'] + ['sst_points'] + ['sst_ipp'] + ['sst_shots'] + ['sst_shooting_pct'] + ['sst_ixg'] + ['sst_icf'] + ['sst_iff'] + ['sst_iscf'] + ['sst_ihdcf'] + ['sst_rush_attempts'] + ['sst_rebounds_created'] + ['sst_penalty_minutes'] + ['sst_penalties_drawn'] + ['sst_giveaways'] + ['sst_takeaways'] + ['sst_hits'] + ['sst_hits_taken'] + ['sst_shots_blocked'] + ['sst_faceoffs_won'] + ['sst_faceoffs_lost'] + ['sst_faceoff_pct']])
    ...
    #Save model
    datum = utils.getTodaysDate(format = "%Y-%m-%d",backdate = None)
    filename = 'DT_' + str(md) + 'MD_FMSE_0001ccp_SeasonTwoWk_Simplified' + str(r_sq_score) + '_'+ datum + '.sav'
    print(filename)
    pickle.dump(dt, open(filename, 'wb'))

def dtTrainSkaterSeasonAndTwoWkTotalsModelSuperSimplified():
    ccp_a = 0.01
    md = 8
    criterion = 'friedman_mse'
    dt = DecisionTreeRegressor(criterion=criterion, max_depth=md, ccp_alpha=ccp_a)
    ...
    query = """SELECT  twowk.goals as twowk_goals,
                            twowk.assists as twowk_assists,twowk.shooting_pct as twowk_shooting_pct,
                            twowk.ixg as twowk_ixg, twowk.icf as twowk_icf, twowk.iff as twowk_iff,
                            twowk.iscf as twowk_iscf, twowk.ihdcf as twowk_ihdcf,
                            sst.goals as sst_goals,
                            sst.assists as sst_assists,sst.shooting_pct as sst_shooting_pct,
                            sst.ixg as sst_ixg, sst.icf as sst_icf, sst.iff as sst_iff,
                            sst.iscf as sst_iscf, sst.ihdcf as sst_ihdcf,
                            sgd.goals as res_goals, sgd.assists as res_assists
             FROM   games g, skater_game_data sgd, skater_two_wk_totals twowk, skater_season_totals sst
             WHERE  g.id = sgd.game_id
             and    g.date >= (?)
    ...
    X = df.drop(columns = ['res_goals', 'res_assists'])
    Y = df[['res_goals', 'res_assists']]
    X_train, X_test, Y_train, Y_test = train_test_split(X, Y, test_size = 0.2, random_state=7) # 80%/20% training/test split
    #print(train[['twowk_games_played'] + ['twowk_time_on_ice'] + ['twowk_goals'] + ['twowk_assists'] + ['twowk_first_assists'] + ['twowk_second_assists'] + ['twowk_points'] + ['twowk_ipp'] + ['twowk_shots'] + ['twowk_shooting_pct'] + ['twowk_ixg'] + ['twowk_icf'] + ['twowk_iff'] + ['twowk_iscf'] + ['twowk_ihdcf'] + ['twowk_rush_attempts'] + ['twowk_rebounds_created'] + ['twowk_penalty_minutes'] + ['twowk_penalties_drawn'] + ['twowk_giveaways'] + ['twowk_takeaways'] + ['twowk_hits'] + ['twowk_hits_taken'] + ['twowk_shots_blocked'] + ['twowk_faceoffs_won'] + ['twowk_faceoffs_lost'] + ['twowk_faceoff_pct'] + ['sst_games_played'] + ['sst_time_on_ice'] + ['sst_goals'] + ['sst_assists'] + ['sst_first_assists'] + ['sst_second_assists'] + ['sst_points'] + ['sst_ipp'] + ['sst_shots'] + ['sst_shooting_pct'] + ['sst_ixg'] + ['sst_icf'] + ['sst_iff'] + ['sst_iscf'] + ['sst_ihdcf'] + ['sst_rush_attempts'] + ['sst_rebounds_created'] + ['sst_penalty_minutes'] + ['sst_penalties_drawn'] + ['sst_giveaways'] + ['sst_takeaways'] + ['sst_hits'] + ['sst_hits_taken'] + ['sst_shots_blocked'] + ['sst_faceoffs_won'] + ['sst_faceoffs_lost'] + ['sst_faceoff_pct']])
    ...
    #Save model
    datum = utils.getTodaysDate(format = "%Y-%m-%d",backdate = None)
    filename = 'DT_' + str(md) + 'MD_FMSE_0001ccp_SeasonTwoWk_SuperSimplified' + str(r_sq_score) + '_'+ datum + '.sav'
    print(filename)
    pickle.dump(dt, open(filename, 'wb'))

def dtTrainSkaterSeasonAndTwoWkTotalsModelSuperSUPERSimplified():
    ccp_a = 0
    md = 3
    criterion = 'friedman_mse'
    dt = DecisionTreeRegressor(criterion=criterion, max_depth=md, ccp_alpha=ccp_a)
    ...
    query = """SELECT  twowk.goals as twowk_goals, sst.goals as sst_goals, sgd.goals as res_goals
             FROM   games g, skater_game_data sgd, skater_two_wk_totals twowk, skater_season_totals sst
             WHERE  g.id = sgd.game_id
             and    g.date >= (?)
             and    g.date <= (?)
    ...
    #Save model
    datum = utils.getTodaysDate(format = "%Y-%m-%d",backdate = None)
    filename = 'DT_' + str(md) + 'MD_FMSE_0001ccp_SeasonTwoWk_SuperSUPERSimplified' + str(r_sq_score) + '_'+ datum + '.sav'
    print(filename)
    pickle.dump(dt, open(filename, 'wb'))

def trainLinearSkaterGoalModelSingleParam():
    linreg = LinearRegression()
    conn = utils.establishDatabaseConnection("main.db")
...
def knnTrainSkaterSeasonAndTwoWkModel(n_neighbors):
    ...
    #Save model
    datum = utils.getTodaysDate(format = "%Y-%m-%d",backdate = None)
    filename = 'KNN_' + str(n_neighbors) + '_SeasonTwoWk' + str(score) + '_'+ datum + '.sav'
    print(filename)
    pickle.dump(knn, open(filename, 'wb'))

def knnTrainSkaterSeasonPPTwoWkOppGoalieTotalsModel(n_neighbors):
    knn = KNeighborsRegressor(n_neighbors=n_neighbors)
    conn = utils.establishDatabaseConnection("main.db")
    ...
    #Save model
    datum = utils.getTodaysDate(format = "%Y-%m-%d",backdate = None)
    filename = 'KNN_' + str(n_neighbors) + '_SeasonPPTwoWkOppGoalie_' + str(score) + '_'+ datum + '.sav'
    print(filename)
    pickle.dump(knn, open(filename, 'wb'))

def rfTrainSkaterSeasonAndTwoWkModel(estimators, pruning, md):
    rf = RandomForestRegressor(n_estimators=estimators, ccp_alpha=pruning, max_depth=md)
    conn = utils.establishDatabaseConnection("main.db")
    ...
    #Save model
    datum = utils.getTodaysDate(format = "%Y-%m-%d",backdate = None)
    filename = 'RF_SeasonTwoWk_' + str(score) + '_MD'+ str(md) + '_Estim'+ str(estimators) + '_' + str(pruning) + '_' + datum + '.sav'
    print(filename)
    pickle.dump(rf, open(filename, 'wb'))

def knnTrainGoalieSeasonTwoWkModel(n_neighbors):
    knn = KNeighborsRegressor(n_neighbors=n_neighbors)
    conn = utils.establishDatabaseConnection("main.db")
    ...
    #Save model
    datum = utils.getTodaysDate(format = "%Y-%m-%d",backdate = None)
    filename = 'KNN_' + str(n_neighbors) + '_GoalieSeasonTwoWk_' + str(r_sq_score) + '_'+ datum + '.sav'
    print(filename)
    pickle.dump(knn, open(filename, 'wb'))

def knnTrainGoalieSeasonTwoWkModelSavesAndGA(n_neighbors):
    knn = KNeighborsRegressor(n_neighbors=n_neighbors)
    conn = utils.establishDatabaseConnection("main.db")
    ...
    query = """SELECT  twowk.gp as twowk_games_played, twowk.toi as twowk_time_on_ice, (twowk.shots_against / twowk.toi * 60) as twowk_sa_p60,
                        ...
                        (gst.ld_shots_against / gst.toi * 60) as gst_ldsa_p60,
                        (gst.ld_saves / gst.toi * 60) as gst_lds_p60, gst.ldsv_pct as gst_ldsv_pct, gst.ldgaa as gst_ldgaa, gst.ldgsaa as gst_ldgsaa,
                        ggd.saves as res_saves, (ggd.shots_faced - ggd.saves) as res_goals_allowed
             FROM   games g, goalie_game_data ggd, goalie_two_wk_totals twowk, goalie_season_totals gst
             WHERE  g.id = ggd.game_id
             and    g.date >= (?)
             and    g.date <= (?)
    ...
    #Save model
    datum = utils.getTodaysDate(format = "%Y-%m-%d",backdate = None)
    filename = 'KNN_' + str(n_neighbors) + '_GoalieSeasonTwoWkSavesAndGA_' + str(r_sq_score) + '_'+ datum + '.sav'
    print(filename)
    pickle.dump(knn, open(filename, 'wb'))


def rfTrainGoalieSeasonAndTwoWkModel(estimators, pruning, md):
    rf = RandomForestRegressor(n_estimators=estimators, ccp_alpha=pruning, max_depth=md)
    conn = utils.establishDatabaseConnection("main.db")
    ...
    #Save model
    datum = utils.getTodaysDate(format = "%Y-%m-%d",backdate = None)
    filename = 'RF_GoalieSeasonTwoWk_' + str(score) + '_MD'+ str(md) + '_Estim'+ str(estimators) + '_' + str(pruning) + '_' + datum + '.sav'
    print(filename)
    pickle.dump(rf, open(filename, 'wb'))

def rfTrainGoalieSeasonAndTwoWkModelSavesAndGA(estimators, pruning, md):
    rf = RandomForestRegressor(n_estimators=estimators, ccp_alpha=pruning, max_depth=md)
    conn = utils.establishDatabaseConnection("main.db")
    ...
    #Save model
    datum = utils.getTodaysDate(format = "%Y-%m-%d",backdate = None)
    filename = 'RF_GoalieSAVESANDGASeasonTwoWk_' + str(score) + '_MD'+ str(md) + '_Estim'+ str(estimators) + '_' + str(pruning) + '_' + datum + '.sav'
    print(filename)
    pickle.dump(rf, open(filename, 'wb'))

def plotRandomForestSingleTree(model_name):
    rf = loadModel(model_name)
    feature_names = ['twowk_games_played', 'twowk_time_on_ice', 'twowk_goals', 'twowk_assists', 'twowk_first_assists',
                     'twowk_second_assists', 'twowk_points', 'twowk_ipp', 'twowk_shots', 'twowk_shooting_pct', 'twowk_ixg',
                     'twowk_icf', 'twowk_iff', 'twowk_iscf', 'twowk_ihdcf', 'twowk_rush_attempts', 'twowk_rebounds_created',
                     'twowk_penalty_minutes', 'twowk_penalties_drawn', 'twowk_giveaways', 'twowk_takeaways', 'twowk_hits',
    ...
    # plot the feature importance
    #feature_plot = feature_importance.plot(kind='bar')
    forest_trees = rf.estimators_
    tree_in_forest = forest_trees[0]
    feat_importances = pd.Series(tree_in_forest.feature_importances_, index=feature_names)
    feat_importances.nlargest(6).plot(kind='barh', fontsize=12, figsize=(5,5)).invert_yaxis()
#     tree_plot = plot_tree(tree_in_forest,
#                 feature_names=feature_names,
#                 filled=True,
#                 rounded=True)
    plt.tight_layout()
...
#trainGoalieSeasonAndTwoWkTotalsModel2Outputs()
...
#analyzeLinearRegModel("Linear_Regression_0.242_2023-04-09")
#trainSkaterSeasonAndTwoWkTotalsModel()
# trainSkaterPlainGoalsOnlyModel()
# trainSkaterSeasonAndTwoWkTotalsModelSimplified()
...
#plotDecisionTreeRegressor("DT_8MD_FMSE_0001ccp_SeasonTwoWk_0.134_2023-05-04")
...
# knnTrainSkaterSeasonAndTwoWkModel(120)
#knnTrainSkaterSeasonPPTwoWkOppGoalieTotalsModel(100)

#Goalie KNNs
# knnTrainGoalieSeasonTwoWkModel(100)
#knnTrainGoalieSeasonTwoWkModelSavesAndGA(100)

#Skater random forest regressor
#rfTrainSkaterSeasonAndTwoWkModel(estimators = 20, pruning = 0.001, md=8)

# Goalie RFs
# rfTrainGoalieSeasonAndTwoWkModel(estimators = 20, pruning = 0.001, md=6)
#rfTrainGoalieSeasonAndTwoWkModelSavesAndGA(estimators = 20, pruning = 0.001, md=5)

plotRandomForestSingleTree("RF_SeasonTwoWk_0.167_20_0.001_2023-05-09")